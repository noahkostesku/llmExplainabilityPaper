"""
===============================================================================
Gemma 3 4B Fine-tuned GAT Loan Default Explanation and Evaluation Pipeline
===============================================================================

This file implements a comprehensive evaluation framework for loan default predictions using GATs with fine-tuned Gemma 3 4B language model explanations focused on network 
...
class Gemma3Config:
    ...
    GEMMA_MODEL_PATH = "<my-path>/gemma-3-4b-it-finetuned-gat-8bit"
...
class Config:
    ...
    EXPLANATIONS_FILE_NAME = "gemma_gnn_explanations.jsonl"
...
GNN explanations are expected to leverage both node features and graph structure."""
...
- 5: Mentions all top 3 most important individual borrower features meaningfully (e.g., "credit score" for "fico")
- 4: Covers 2 of top 3 important individual borrower features with good detail, minor gaps
- 3: Covers some important individual borrower features but misses key ones or includes less important ones  
...
- For {model_type.upper()} models, {"both individual and network factors should be well-represented" if model_type.lower() == "gnn" else "focus should be on individual feature importance"}
...
def get_gnnexplainer_insights_for_node(model, graph_data, node_idx_to_explain, num_top_features, num_top_edges):
    ...
    print(f"Explaining node {node_idx_to_explain}...")
...
def format_masks_to_text(explanation_data):

    top_features = explanation_data['top_features_impact']
    top_edges = explanation_data['top_edges_impact']
    actual_values = explanation_data.get('actual_feature_values', {})
    
    node_text = ""
    for feature, importance in top_features.items():
        actual_val = actual_values.get(feature, 'N/A')
        direction = "increases" if importance > 0 else "decreases"
        node_text += f"- {feature} (value: {actual_val:.4f}): {direction} default risk (network importance: {importance:.4f})\n"

    edge_text = ""
    if top_edges and any(abs(edge['importance']) > 1e-6 for edge in top_edges):
        for edge in top_edges:
            if abs(edge['importance']) > 1e-6:  
                src = edge['source']
                dst = edge['destination']
                imp = edge['importance']
                attrs = edge['attributes']
                
                conn_type = "network connection"
                if attrs.get('edge_type_area_connection', 0) == 1.0:
                    conn_type = "geographic area connection"
                elif attrs.get('edge_type_provider_connection', 0) == 1.0:
                    conn_type = "same loan provider connection"
                elif attrs.get('edge_type_area_provider_connection', 0) == 1.0:
                    conn_type = "area-provider cluster connection"
                
                group_size = 1.0 / attrs.get('inverse_group_size', 1.0)
                edge_text += f"- Connection from loan {src} to {dst} ({conn_type}) with importance {imp:.4f} (cluster size: ~{group_size:.0f} loans)\n"
    
    if not edge_text:
        edge_text = "- Network connections show minimal impact on the prediction (all edge importances near zero).\n"
        
    return node_text, edge_text

def build_gemma_prompt_for_gnn_explanation(explanation_data, df_row=None):
...
def initialize_gemma3():
    ...
    try:
        ...
        print(f"Loading LoRA adapter from: {config.GEMMA_MODEL_PATH}")
...
def get_gemma3_response(system_prompt, user_prompt, model, tokenizer):
...
if __name__ == '__main__':
    ...
    try:
        gemma_model, gemma_tokenizer, success = initialize_gemma3()
        if not success:
            print("Failed to initialize Gemma3 model. Exiting...")
            sys.exit(1)
        ...
        print("Loading dataframe for feature translations...")
        ...
        if selected_node_indices is None:
            ...
            selected_node_indices = selected_node_indices[:100]
        ...
        for i, node_idx_to_explain in enumerate(selected_node_indices):
            ...
            system_prompt, user_prompt = build_gemma_prompt_for_gnn_explanation(explanation_results, df_row)
            gemma3_response = get_gemma3_response(system_prompt, user_prompt, gemma_model, gemma_tokenizer)

            print("\n### Gemma3's Explanation ###")
            
            print(gemma3_response)

            perplexity = None
            try:
                print("\nCalculating perplexity...")
                perplexity = calculate_perplexity_with_gpt_neo(gemma3_response)
            ...
            try:
                ...
                grades = llm_grade_explanation(gemma3_response, explanation_results)
                if grades:
