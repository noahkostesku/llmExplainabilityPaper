"""
===============================================================================
Gemini 2.5 GAT Loan Default Explanation and Evaluation Pipeline
===============================================================================

This file implements a comprehensive evaluation framework for loan default predictions using GATs with Gemini 2.5 language model explanations focused on network 
...
class Config:
    ...
    EXPLANATIONS_FILE_NAME = "gemini_gnn_explanations.jsonl"
...
- For GNN models, both individual and network factors should be well-represented
...
def build_gemini_prompt_for_gnn_explanation(explanation_data, df_row=None):
...
def get_gemini_response(system_prompt, user_prompt):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Gemini REST API error {response.status_code}: {response.text}")
            return None
...
if __name__ == '__main__':
    ...
    try:
        ...
        all_perplexities = []
        all_individual_coverage_scores = []
        all_individual_consistency_scores = []
        ...
        for i, node_idx_to_explain in enumerate(selected_node_indices):
            ...
            system_prompt, user_prompt = build_gemini_prompt_for_gnn_explanation(
                explanation_results, 
                df_row
            )
            ...
            try:
                ...
                perplexity = calculate_perplexity_with_gpt_neo(gemini_response)
            ...
            grades = None
            try:
                print("\nGrading explanation with 4-metric LLM-as-a-Judge...")
                grades = llm_grade_explanation(gemini_response, explanation_results, model_type="gnn")
                if grades:
                    print(f"GAT LLM-as-a-Judge Scores: {grades}")
            except Exception as e:
                print(f"Error during LLM grading: {e}")

            if perplexity is not None and grades:
                all_perplexities.append(perplexity)
                all_individual_coverage_scores.append(grades["Individual Feature Coverage"])
                all_individual_consistency_scores.append(grades["Individual Feature Consistency"])
        ...
        print(f"Individual Feature Coverage: {len(all_individual_coverage_scores)} samples")
        print(f"Individual Feature Consistency: {len(all_individual_consistency_scores)} samples")
        ...
        min_samples = min(len(all_perplexities), len(all_individual_coverage_scores), 
                         len(all_individual_consistency_scores), len(all_network_coverage_scores), 
                         len(all_network_consistency_scores))

        print("\n### 95% Confidence Intervals ###")
        for name, data in [
            ("Perplexity", all_perplexities),
            ("Individual Feature Coverage", all_individual_coverage_scores),
            ("Individual Feature Consistency", all_individual_consistency_scores),
