...
def initialize_proxies():
    ...
    if http_proxy:
        ...
        print(f"[Proxy] HTTP_PROXY set to: {http_proxy}")
    else:
        print("[Proxy] HTTP_PROXY not set in environment")
    
    if https_proxy:
        os.environ["HTTPS_PROXY"] = https_proxy
        print(f"[Proxy] HTTPS_PROXY set to: {https_proxy}")
    else:
        print("[Proxy] HTTPS_PROXY not set in environment")
...
class GeminiHybridPipeline:
    
    def __init__(self, config=None):
        
        self.config = config or GeminiHybridConfig()
        self.gat_model = None
        self.xgb_model = None
    ...
    def initialize_gemini(self):
        ...
        print("Initializing Gemini via HTTP API...")
        load_dotenv()
        ...
        self.gemini_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={api_key}"

        print("Gemini HTTP API initialized.")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Gemini API test failed with status code: {response.status_code}")
                return False
    ...
    def get_gnn_explanation(self, node_idx):
        ...
        if self.gnn_explainer is None:
            print("GNN explainer not available")
            return {}, "GNN explainer not available"
        try:
    ...
    def get_hybrid_prediction(self, node_idx):
        ...
        hybrid_proba = (self.config.HYBRID_WEIGHT_GAT * gat_proba + self.config.HYBRID_WEIGHT_XGB * xgb_proba)
    ...
    def build_enhanced_gemini_prompt(self, predictions, shap_info, gnn_info):
    ...
    def get_gemini_explanation_enhanced(self, predictions, shap_info, gnn_info):

        try:
            system_prompt, user_prompt = self.build_enhanced_gemini_prompt(predictions, shap_info, gnn_info)
            ...
            try:
                if 'candidates' in data and len(data['candidates']) > 0:
                    ...
                    if 'finishReason' in candidate and candidate['finishReason'] == 'SAFETY':
                        return "Error: Content was blocked by Gemini safety filters"
                ...
                print(f"Key error parsing response: {e}")
            ...
            return "Error: Connection error to Gemini API"
        except Exception as e:
            print(f"Unexpected error in Gemini explanation: {str(e)}")
            import traceback
            traceback.print_exc()
            return f"Error: Unexpected error generating explanation ({str(e)})"
    ...
    def get_comprehensive_explanation(self, node_idx):
        ...
        try:
            ...
            gemini_explanation = self.get_gemini_explanation_enhanced(
                predictions, (shap_features, shap_summary), (gnn_data_for_translation, network_summary)
            )

            if gemini_explanation.startswith("Error:"):
                print(f"Gemini explanation failed: {gemini_explanation}")
                approval_chance = (1 - predictions['hybrid_proba']) * 100
                gemini_explanation = f"Technical analysis shows {approval_chance:.0f}% approval likelihood based on credit profile and network patterns. Detailed explanation unavailable due to API issues."
    ...
    def run_hybrid_demo(self, node_idx=None, num_samples_perplexity=None, selected_indices=None):
        ...
        explanation_file = "explanations/gemini_hybrid_explanations.jsonl"
        ...
        if selected_indices is None:
            if node_idx is not None:
                ...
                print(f"\n### Using fixed node {node_idx} for all {num_samples_perplexity} samples ###")
                node_indices = [node_idx] * num_samples_perplexity
            else:
        ...
        for i, current_node_idx in enumerate(node_indices):
            ...
            try:
                ...
                if shap_features:
                    ...
                    print(f"Node {current_node_idx} Grading: SKIPPED (no SHAP features)")
