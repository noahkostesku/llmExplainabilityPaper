"""
===============================================================================
Gemma 3 Fine-tuned 4B XGB Loan Default Explanation and Evaluation Pipeline
===============================================================================

This module implements a comprehensive evaluation framework for loan default predictions using XGBoost models with fine-tuned Gemma 3 4B language model explanations. The system generates human-readable explanations of individual borrower risk assessments and evaluates explanation quality through automated scoring metrics including perplexity analysis and LLM-based grading.

The pipeline focuses on individual borrower characteristics (FICO scores, debt-to-income ratios, loan-to-value ratios, etc.) and generates structured explanations that prioritize feature alignment and directional accuracy in risk assessment.
...
EVAL_SAVE_DIR = '../evaluations/gemma3_xgb/'
...
class Gemma3Config:
    GEMMA_MODEL_PATH = "<my-path>/gemma-3-4b-it-finetuned-xgb-8bit"
...
def llm_grade_explanation(explanation, insight, model_type="xgb"):
    ...
    model_context = """For XGB models: The explanation should focus on individual borrower characteristics and 
their importance in the decision. Traditional machine learning explanations emphasize feature importance."""
...
def build_gemma3_prompt_for_single_borrower(borrower_shap_insight, optimized_threshold, X_test=None, borrower_idx=None):
...
def evaluate_explanations_with_scoring(model, X_test, y_test, optimized_threshold, selected_indices=None, num_samples=10):
    ...
    print(f"\n### Starting Explanation Evaluation for {len(selected_indices) if selected_indices else num_samples} samples ### ")
    ...
    explanations_file = os.path.join(explanations_dir, "gemma_xgb_explanations.jsonl")
    ...
    for i, idx in enumerate(selected_indices):
        ...
        try:
            ...
            system_prompt, user_prompt = build_gemma3_prompt_for_single_borrower(
    insight, optimized_threshold, X_test=X_test, borrower_idx=idx
)
            explanation = get_gemma3_response(system_prompt, user_prompt, gemma_model, gemma_tokenizer)
...
if __name__ == "__main__":
    ...
    model, X_test, y_test, optimized_threshold, selected_indices
    )
    ...
    for dimension, scores in evaluation_stats['grading_scores'].items():
        if scores:
            ...
            if len(unique_scores) <= 3:
                ...
                print(f"Distribution: {score_counts}")
