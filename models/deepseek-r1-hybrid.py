"""
===============================================================================
DeepSeek-R1 70B Fine-tuned Hybrid Loan Default Explanation and Evaluation Pipeline
===============================================================================

This file implements a comprehensive machine learning pipeline that combines GATs
and XGBoost models to predict loan defaults using both individual borrower characteristics and network relationship patterns. The system integrates SHAP explainability for tabular features, GNNExplainer for graph relationships, and fine-tuned DeepSeek-R1 70B language models to generate human-readable explanations of prediction decisions.
...
class GAT(torch.nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, heads, attn_dropout=0.6, feat_dropout=0.5, edge_dim=None):
...
class DeepSeekHybridConfig:
    
    DEEPSEEK_MODEL_PATH = "<my-path>/deepseek-r1-70b-it-finetuned-hybrid-8bit"
...
class DeepSeekHybridPipeline:
    
    def __init__(self, config=None):
        
        self.config = config or DeepSeekHybridConfig()
        self.gat_model = None
        self.xgb_model = None
    ...
    def initialize_deepseek(self): 
        ...
        try:
            ...
            print(f"Loading LoRA adapter: {self.config.DEEPSEEK_MODEL_PATH}")
            self.deepseek_model = PeftModel.from_pretrained( 
                base_model, 
    ...
    def initialize_explainers(self):
        ...
        try:
            if self.xgb_model is not None and self.X_test is not None:
                background_sample = self.X_test[self.config.XGB_TRAINED_FEATURES].sample(n=min(100, len(self.X_test)), random_state=42)
    ...
    def _prepare_xgb_input(self, node_idx):
        ...
        if len(available_features) != len(self.config.XGB_TRAINED_FEATURES):
            return None
        return single_instance[self.config.XGB_TRAINED_FEATURES]
    
    def build_enhanced_hybrid_prompt(self, predictions, shap_info, gnn_info):
        
        hybrid_proba = predictions['hybrid_proba']
        shap_features, shap_summary = shap_info
        gnn_explanation, network_summary = gnn_info

        current_node_idx = getattr(self, '_current_node_idx', 0)
        df_row = self.X_test.iloc[current_node_idx] if (hasattr(self, 'X_test') and current_node_idx < len(self.X_test)) else None

        customer_factors = []
        top_factors_formatted = []
        factor_count = 1
        for feature, feature_data in shap_features.items():
        ...
        if network_summary:
            top_factors_formatted.append(f"{factor_count}. {network_summary}")
            factor_count += 1

        network_context = self._translate_network_to_customer_language(gnn_explanation)
        ...
        user_prompt = f"""
    You are explaining to someone why their loan application has {approval_context} after analyzing both their personal finances AND their connections to other borrowers in their area.

    COMPLETE FINANCIAL PICTURE:
    - Personal finances: {', '.join(customer_factors[:3])}
    - Network context: {network_context}

    DETAILED BREAKDOWN - You MUST discuss all of these personal financial factors and also your network connections with equal attention:
    {chr(10).join(top_factors_formatted)}

    WRITE A COMPREHENSIVE EXPLANATION (8–10 sentences) following these rules:
    1. Start by explaining what the approval chance means (e.g., "you have an X% chance of approval means...").
    2. **Personal Factors** – For each factor above (credit score, debt ratio, etc.), explain *why* it matters and how it affects the loan terms. (Give concrete comparisons like "better than Y% of applicants".)
    3. **Network/Area Factors** – Explain the network connections (e.g., "connected to 3 other borrowers"). Use exact numbers and describe what these connections imply (job market, housing, etc.).
    4. **Combined Analysis** – Describe how personal and network factors together influence approval. If they conflict, explain the difference and which carries more weight.
    5. **Actionable Advice** – Suggest improvements: for personal factors (e.g. debt/payments) and for network factors (e.g. maintain local economic strengths).
    6. **Comparison Perspective** – Compare the applicant to peers ("others your age", "people in your area").
    7. Conclude on a positive note about the overall financial outlook.

    CRITICAL: Cover *all* listed factors (and connections) equally; do not skip any. Do not use technical terms like "model", "algorithm", or "SHAP". Use clear, non-technical language and specific numbers throughout.
    """
        return GLOBAL_SYSTEM_PROMPT, user_prompt

    
    def build_enhanced_deepseek_prompt(self, predictions, shap_info, gnn_info):

        return self.build_enhanced_hybrid_prompt(predictions, shap_info, gnn_info)
    
    def get_deepseek_explanation_enhanced(self, predictions, shap_info, gnn_info):
        if self.deepseek_model is None or self.deepseek_tokenizer is None:
            return "DeepSeek explanation not available - model not initialized"

        system_prompt, user_prompt = self.build_enhanced_hybrid_prompt(predictions, shap_info, gnn_info)
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user",   "content": user_prompt}
        ]
        formatted_prompt = self.deepseek_tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        inputs = self.deepseek_tokenizer(formatted_prompt, return_tensors="pt", truncation=True, max_length=8000, padding=True)
        device = next(self.deepseek_model.parameters()).device
        ...
        with torch.inference_mode():
            outputs = self.deepseek_model.generate(
                ...
                top_p=1.0,
                top_k=0,
                pad_token_id=self.deepseek_tokenizer.pad_token_id,
                eos_token_id=self.deepseek_tokenizer.eos_token_id,
            )
        generated = outputs[:, input_ids.shape[1]:]
        explanation = self.deepseek_tokenizer.decode(generated[0], skip_special_tokens=True, clean_up_tokenization_spaces=True).strip()

        stop_patterns = [
            "GUIDED SUBGOAL STRUCTURE", "OUTPUT LENGTH/FORMAT CONSTRAINT",
            "COMBINED BORROWER-LEVEL", "COMBINED INSTRUCTIONAL", "CHAIN-OF-THOUGHT",
            "DISAMBIGUATION HINT"
        ]
    ...
    def _translate_network_to_customer_language(self, gnn_data):
        ...
        if not top_edges:
            # Try to extract some network info from the graph data itself
    ...
    def get_comprehensive_explanation(self, node_idx):
        ...
        try:
            ...
            deepseek_explanation = self.get_deepseek_explanation_enhanced(
                predictions, (shap_features, shap_summary), (gnn_data_for_translation, network_summary)
            )

            return deepseek_explanation.strip()
    ...
    def run_hybrid_demo(self, node_idx=None, num_samples_perplexity=None, selected_indices=None):
        ...
        explanation_file = "explanations/deepseek_hybrid_explanations.jsonl"
        ...
        if selected_indices is not None:
            ...
            node_indices = valid_indices[:num_samples_perplexity]  
            print(f"Using {len(node_indices)} valid indices from selected_indices")
        ...
        if selected_indices is None:
            if node_idx is None:
                ...
                for i in range(num_samples_perplexity):
                    ...
                    if len(node_indices) == i:
                        if visited_nodes:
                            fallback_idx = random.choice(list(visited_nodes))
                            node_indices.append(fallback_idx)
        ...
        for i, current_node_idx in enumerate(node_indices):
            ...
            try:
                ...
                grades = llm_grade_explanation(explanation, insight_for_grading)
                if grades:
        ...
        for name, data in [
            ...
            if name == "Perplexity" and len(data) > 1 and np.std(data) > 0:
                ...
                if mean_score - h < 1.0:
                    ...
                    print(f"{name}: {ci_str}")
            elif name in ["Individual Feature Coverage", "Individual Feature Consistency", "Network Coverage", "Network Consistency"] and len(data) > 1 and np.std(data) > 0:
                h = stats.sem(data) * stats.t.ppf(0.975, len(data) - 1)
                ci_low_bounded = max(1.0, mean_score - h)
...
if __name__ == "__main__":
    ...
    pipeline = DeepSeekHybridPipeline() 
